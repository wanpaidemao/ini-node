...
func (b *BlockChain) initChainState() error {
	...
	// A3 watermark: the highest height whose chain-state metadata was
	// durably written by the async write queue.  With v1 (block bodies
	// stored synchronously, metadata asynchronously and atomically with the
	// watermark), a crash leaves the DB consistent exactly at the watermark
	// -- the persisted best state and the watermark advance in the same
	// transaction, so the loaded tip never sits above the watermark.  No
	// replay is therefore needed on startup: the sync machinery resumes from
	// the watermark height and re-connects the already-on-disk bodies, which
	// regenerates the missing metadata rows.  The watermark is logged here
	// for diagnostics; a full replay loop is deferred to v2 (block bodies
	// also async), see design doc 3.3.2.
	// A3 水印:异步写队列已持久化链状态元数据的最高高度。v1 下(块体同步
	// 存储、元数据异步且与水印同事务原子推进)崩溃后 DB 恰好在水印处
	// 一致——持久化 best state 与水印在同一事务推进,加载的 tip 不会高于
	// 水印。因此启动无需重放:同步机制从水印高度恢复,重连已在磁盘的
	// 块体,自动重新生成缺失的元数据行。此处记录水印供诊断;完整重放
	// 循环留到 v2(块体也异步),见设计文档 3.3.2。
	if b.writeQueue != nil {
		var watermark int32 = -1
		wErr := b.db.View(func(dbTx database.Tx) error {
			var err error
			watermark, err = loadWriteWatermark(dbTx)
			return err
		})
		if wErr != nil {
			log.Warnf("A3: failed to load write watermark: %v", wErr)
		} else if tip := b.bestChain.Tip(); tip != nil && watermark < tip.height {
			log.Infof("A3: write watermark %d below loaded tip %d -- "+
				"metadata tail will be regenerated by sync re-connect",
				watermark, tip.height)
		} else {
			log.Infof("A3: write watermark %d", watermark)
		}
	}
