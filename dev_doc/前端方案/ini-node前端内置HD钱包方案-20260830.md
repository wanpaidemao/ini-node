...
| **Step 6** ✅ | 代币接入：tokenapi 包（新增）+ token 代理 RPC（新增） | ✅（2026-08-31）与 6b 合并实现：代币不走节点 RPC，改由前端进程内 `frontend/gotoken.go` + `TokenService` bindings 直连 tokenstest REST（余额/info/params），节点零改动 |
| **Step 6b** ✅ | **代币四操作对齐**（§ 功能差距清单 P1）：移植 web-wallet `internal/api/token.go` + `TokenTransfer/Create/Issue/Burn` bindings → Tokens tab 真实 UI | 🔶（2026-08-31）代码完成待实机验证：`frontend/gotoken.go` 移植 web-wallet `internal/api/token.go` + `internal/tx` 代币构造/签名/广播（`buildSignBroadcastToken` 复用 gosend 的 UTXO 两级降级与找零逻辑）；`Balances/Info/Transfer/Create/Issue/Burn` 6 bindings（main.go 注册 TokenService）；Wallet.svelte 新增 Tokens tab（余额表 + transfer/create/issue/burn 四操作面板）；钱包设置新增代币 API 地址项（默认 tokenstest.sugar.wtf）；i18n×9。待 tokenstest 联调 |
...
| **Step 9** ✅ | **地址三型切换（P1）**：§5.12——移植 internal/address 三型派生 + AddressFor binding + 设置项即时生效 | 🔶（2026-08-31）代码完成待实机验证：`frontend/goaddress.go`（DeriveAll 三型：bech32=P2WPKH、segwit=P2SH-P2WPKH、legacy=P2PKH，同一密钥材料）；`AddressFor(index, type)` binding；gosend.go UTXO 查询按当前地址类型取对应 script；钱包设置新增地址类型下拉（即时生效：钱包页地址/QR/余额/Keys 全刷新，getAddressFor 按型派生）；i18n×9。待 sugarindex 三型余额验证 |
| **Step 10** ✅ | **Explorer（P2）**：§5.13——三级路由 + 本节点 RPC 数据 + 历史 txid 内链 | 🔶（2026-08-31）代码完成待实机验证：`Explorer.svelte` 三级视图（链统计+区块列表 → 区块详情 → 交易详情，`view` 状态机 + 面包屑返回）；数据走本节点 RPC（getblockchaininfo/getblockhash/getblock/verbose=2/getrawtransaction verbose）；搜索框支持高度/区块哈希/txid 自动分流；`navigate()` 扩展 params 深链（app.shortcut），App.svelte 注册 explorer 路由+导航；Wallet 页历史 txid 点击内链 → Explorer 交易详情。交易详情完整内容依赖节点 `txindex=1`。i18n×9 |
| **Step 11** ✅ | P3 收尾：WIF 导入（OpenWalletKey 对应 binding）+ Broadcast 并入控制台 | 🔶（2026-08-31）代码完成待实机验证：`backend/wallet/wif.go`（WIF 解析→内存会话，index 0 地址与 web-wallet OpenWalletKey 一致）；`Manager.LoginWIF` + `WalletService.LoginWIF/ExportWIF` bindings；Create.svelte 新增 "WIF Key" 第三登录标签；Keys tab 每行 WIF 导出按钮（校验+复制）。i18n×9 |
